...
int main(int argc, char *argv[])
{
    ...
    for (int t = 0; t < N_THREADS; t++)
    {
        ...
        std::cout << ' ' << samples;
        threads[t] = std::thread(compute_rays, t, std::ref(bounce_counts[t]), samples_left, std::ref(framebuffer), width, height, samples, MAX_BOUNCES, cam, world, trace_probability, array_of_paths);
    }
    std::cout << " done.\n";
